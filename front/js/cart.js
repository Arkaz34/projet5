...
function afficherPanier() {
    ...
    //sinon 
        ...
        panier.forEach(function (canape) {
            //fetch récupére les données de l'API du "Catalogue de canapés"
...
btnEnvoyerCommande.addEventListener("click", function (e) {
    e.preventDefault();
    //----------------------
    /*Récupérer et analyser les données saisies par l’utilisateur dans le "contact".
    ...
    //----------------------
    //contôle de la validité contact
    //indiquer le prénom
    ...
    if (panier == "") {
        ...
        panier.forEach(function (el) {
            ...
            //envoie de l'oblet vers le serveur
            const options = {
                method: 'POST',
                    ...
                    .then(function (res) {
                        res.json()
                            .then(function (data) {
                                ...
                                window.localStorage.postItem('envoiDonnees', envoiDonnees);
                            })
                    })
            } else {
